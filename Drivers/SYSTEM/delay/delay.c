...
#if SYS_SUPPORT_OS
static uint16_t g_fac_ms = 0;       /* ms延时倍乘数 */

/* 声明 FreeRTOS 内部的中断处理函数 */
extern void xPortSysTickHandler(void);

/**
 * @brief     SysTick中断服务函数
 * @note      该函数在 stm32f4xx_it.c 中也可以实现，但此处提供统一逻辑
 */
void SysTick_Handler(void)
{
    HAL_IncTick(); /* 给 HAL 库用（维持 HAL_GetTick 的计数） */

    /* 如果调度器已经启动，则调用 FreeRTOS 的节拍处理函数 */
...
/**
 * @brief     初始化延迟函数
 * @param     sysclk: 系统时钟频率, 即CPU频率(168MHz就输入168)
 */
void delay_init(uint16_t sysclk)
{
#if SYS_SUPPORT_OS
    uint32_t reload;
#endif
    g_fac_us = sysclk;                          /* 1us需要的计数值 */
    
#if SYS_SUPPORT_OS
    /* 计算每秒需要的 SysTick 加载值 */
    reload = sysclk;                            /* 每秒钟的计数次数 单位M */
    reload *= 1000000 / configTICK_RATE_HZ;     /* 根据 configTICK_RATE_HZ 设定溢出时间 */
    
    g_fac_ms = 1000 / configTICK_RATE_HZ;       /* 代表OS可以延时的最少单位(1ms) */

    /* 此时不直接开启中断，FreeRTOS 启动调度器时会统一配置 */
#endif
}

/**
 * @brief     延时nus (微秒)
 * @note      采用时钟摘取法，不引起任务切换
 * @param     nus: 要延时的us数
 */
void delay_us(uint32_t nus)
{
    ...
    while (1)
    {
        ...
        if (tnow != told)
        {
            if (tnow < told)
            {
                tcnt += told - tnow;            /* SYSTICK是一个递减计数器 */
            }
            else
            {
                tcnt += reload - tnow + told;
...
void delay_ms(uint16_t nms)
{
    /* 如果 OS 正在运行且不在中断中，使用 vTaskDelay 释放 CPU */
    if (xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED && 
       ((SCB->ICSR & SCB_ICSR_VECTACTIVE_Msk) == 0)) 
    {
        if (nms >= g_fac_ms)                    /* 延时时间大于一个节拍周期 */
        {
            vTaskDelay(nms / g_fac_ms);         /* FreeRTOS 任务延时 */
        }
        nms %= g_fac_ms;                        /* 剩余不足一个节拍的时间采用普通延时 */
    }
    
    /* 调度器未开启或延时极短，使用普通忙等延时 */
    if (nms > 0)
    {
